...
fn find_move_by_san(pos: &Chess, san: &str) -> Option<Move> {
    let moves = pos.legal_moves();

    for mv in moves {
        let mv_as_san = San::from_move(pos,mv);
        if mv_as_san.to_string() == san {
            return Some(mv);
        }
    }
...
8/8/8/1k6/6b1/4N3/2p3K1/3n4 w - -; Nxd1; -800; N - N
8/8/1k6/8/8/2N1N3/2p1p1K1/3n4 w - -; Ncxd1; -800; N - (N + Q - P)
...
"#;
    ...
    for (i, test) in tests.iter().enumerate() {
        ...
        let pos = read_position_from_fen(&test.fen).unwrap();

        let mv = match find_move_by_san(&pos, &test.san) {
            Some(m) => m,
            None => {
                println!("Test {} FAILED: Move {} not found in legal moves", i, test.san);
                continue;
            }
        };
        ...
        if i32::from(score) == test.expected {
            passed += 1;
        } else {
            println!("Test {} FAILED", i);
            println!("FEN      : {}", test.fen);
            println!("Move     : {}", test.san);
...
fn parse_see_test_suite(input: &str) -> Vec<SeeTest> {
    input
        ...
        .map(|line| {
            ...
            SeeTest {
                fen,
                san: parts[1].to_string(),
                expected: parts[2].parse().unwrap(),
            }
        })
        .collect()
}
