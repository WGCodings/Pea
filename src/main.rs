...
fn main() {
    ...
    // Load in nnue
    static NNUE: Network = unsafe { std::mem::transmute(*include_bytes!("../nnue/mb/run9_7_net_0/run9_7_net_0-10/quantised.bin")) };
    ...
    if let Some("bench") = std::env::args().nth(1).as_deref() {
        ...
        let (_, best_move, pv) = Threads::search(
            ...
            12, 100_000_000, Some(Duration::from_secs(100)), uci_state.stop.clone(),
        );
        return;
    }
